...
class trainer(object):
    ...
    def train(self, opt):
        ...
        for step in range(start_iter, opt.num_iter + 1):
            ...
            tar_char_preds, tar_bpe_preds, tar_wp_preds, tar_global_feature, tar_local_feature = self.model(tar_image)
            ...
            if tar_portion < 1.0:
                ...
                tar_preds_max_prob, tar_preds_class = tar_char_preds.max(dim=1)
                ...
                for c in class_set:
                    ...
                    tar_ent_pool = tar_char_em_loss[mask]
            ...
            # Add domain loss
            loss = src_char_cls_loss.mean() + src_bpe_cls_loss.mean() + src_wp_cls_loss.mean() + (tar_char_em_loss.mean() + tar_bpe_em_loss.mean() + tar_wp_em_loss.mean()) * tar_lambda #* 3 #+ 3 * omega * (d_img_loss.mean() + d_inst_loss.mean()) #tar_em_loss choosed_ent_pool
            loss_avg.add(loss)
            cls_loss_avg.add(src_char_cls_loss + src_bpe_cls_loss + src_wp_cls_loss)
            # sim_loss_avg.add(d_img_loss + d_inst_loss)
            em_loss_avg.add(tar_char_em_loss + tar_bpe_em_loss + tar_wp_em_loss) # tar_em_loss choosed_ent_pool
            ...
            if step % opt.valInterval == 0:
                ...
                print(
                    f'[{step}/{opt.num_iter}] Loss: {loss_avg.val():0.5f} CLS_Loss: {cls_loss_avg.val():0.5f} EM_Loss: {em_loss_avg.val():0.5f} elapsed_time: {elapsed_time:0.5f}') #SIMI_Loss: {sim_loss_avg.val():0.5f}
                # for log
