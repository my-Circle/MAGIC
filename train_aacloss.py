...
from losses.losses import BCE_softlabels, get_losses_unlabeled, advbce_unlabeled
...
class trainer(object):
    ...
    def train(self, opt):
        ...
        for step in range(start_iter, opt.num_iter + 1):
            ...
            src_char_preds, src_bpe_preds,src_wp_preds,src_global_feature, src_local_feature = self.model(src_image)
            ...
            tar_preds, _, _, tar_global_feature, tar_local_feature = self.model(tar_image)
                                                                          # char_tar_text[:, :-1],bpe_tar_text[:,:-1],wp_tar_text[:,:-1],
                                                                          # is_train=False)
            """ aacloss"""
            prob_tar = F.softmax(tar_preds.contiguous().view(-1,tar_preds.shape[-1]), dim=1)
            feat = tar_local_feature.contiguous().view(-1, tar_local_feature.shape[-1])
            aac_loss = advbce_unlabeled(opt, target=None, feat=feat, prob=prob_tar, prob_bar=prob_tar, device=device,bce=self.bce)
            ...
            # Add domain loss
            loss = src_char_cls_loss.mean() + src_bpe_cls_loss.mean() + src_wp_cls_loss.mean() + 3 * omega * (d_img_loss.mean() + d_inst_loss.mean()) + 3 * aac_loss
            loss_avg.add(loss)
            cls_loss_avg.add(src_char_cls_loss + src_bpe_cls_loss + src_wp_cls_loss)
            sim_loss_avg.add(d_img_loss + d_inst_loss)
            aac_avg.add(aac_loss)
            ...
            if step % opt.valInterval == 0:
                ...
                print(
                    f'[{step}/{opt.num_iter}] Loss: {loss_avg.val():0.5f} CLS_Loss: {cls_loss_avg.val():0.5f} SIMI_Loss: {sim_loss_avg.val():0.5f} AAC_Loss: {aac_avg.val():0.5f} elapsed_time: {elapsed_time:0.5f}')
                # for log
