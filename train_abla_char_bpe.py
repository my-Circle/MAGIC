...
class trainer(object):
    ...
    def train(self, opt):
        ...
        for step in range(start_iter, opt.num_iter + 1):
            ...
            # Add domain loss
            loss = src_char_cls_loss.mean() + src_bpe_cls_loss.mean() + src_wp_cls_loss.mean() + (tar_char_em_loss.mean() + tar_bpe_em_loss.mean()) * tar_lambda + omega * (d_img_loss.mean() + d_inst_loss.mean() + d_inst_loss_bpe.mean()) #2str:0.8*tar_char_em_loss.mean() + 0.1*tar_bpe_em_loss.mean() 2iam:tar_char_em_loss.mean() + tar_bpe_em_loss.mean()
            loss_avg.add(loss)
            cls_loss_avg.add(src_char_cls_loss + src_bpe_cls_loss + src_wp_cls_loss)
            sim_loss_avg.add(d_img_loss + d_inst_loss + d_inst_loss_bpe)
