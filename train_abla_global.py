...
class trainer(object):
    ...
    def get_batch_entropy_loss(self, p_softmax):
        entropy = - torch.mul(p_softmax, torch.log(p_softmax + 1e-8))
    ...
    def train(self, opt):
        ...
        for step in range(start_iter, opt.num_iter + 1):
            ...
            # Add domain loss
            loss = src_char_cls_loss.mean() + src_bpe_cls_loss.mean() + src_wp_cls_loss.mean() + omega * (d_img_loss.mean()) + tar_em_loss.mean() * tar_lambda #* 3
            loss_avg.add(loss)
            cls_loss_avg.add(src_char_cls_loss + src_bpe_cls_loss + src_wp_cls_loss)
            sim_loss_avg.add(d_img_loss)
