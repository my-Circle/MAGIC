...
class Attention(nn.Module):
    ...
    def forward(self, batch_H, text, bpe_text, wp_text, is_train=True, batch_max_length=25):
        ...
        if is_train:
            ...
            char_probs = torch.FloatTensor(batch_size, num_steps, self.num_classes).fill_(0).to(device)
            bpe_probs = torch.FloatTensor(batch_size, num_steps, 50257).fill_(0).to(device)
            wp_probs = torch.FloatTensor(batch_size, num_steps, 30522).fill_(0).to(device)

            for i in range(num_steps):
                char_onehots = self._char_to_onehot(targets, onehot_dim=self.num_classes)
                hidden, alpha, context = self.attention_cell(hidden, batch_H, char_onehots)
                char_probs_step = self.char_head(hidden[0])
                char_probs[:, i, :] = char_probs_step
                _, next_input = char_probs_step.max(1)
        ...
        self.alpha_history.permute(0,2,1)  # batch_size x num_steps x num_classes
