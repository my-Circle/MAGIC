...
class TokenLearner(nn.Module):
    ...
    def __init__(self, input_embed_dim, out_token=30):  #out_token:max_seq_len=27
        super().__init__()
    ...
    def forward(self, x): #192,257,768 重点逻辑 对应论文中的A3模块
        B = x.shape[0]
        ...
        x = torch.einsum('...si,...id->...sd', selected, feat) # [bs, 27, 768]  忽略前几维的矩阵乘法
        
        x = self.norm(x)
        return selected, x  #selected:(192,27,257) x(192,27,768)