...
def get_losses_unlabeled(args, G, F1, im_data, im_data_bar, im_data_bar2, target, BCE, w_cons, device):
    ...
    feat = G(im_data)  # 64,4096
    feat_bar = G(im_data_bar)  # 64, 4096
    feat_bar2 = G(im_data_bar2)  # 64, 4096

    output = F1(feat, reverse=True, eta=1.0)  # 64,126
    output_bar = F1(feat_bar, reverse=True, eta=1.0)  # 64, 126
    prob, prob_bar = F.softmax(output, dim=1), F.softmax(output_bar, dim=1)  # 64,126  64,126
...
def pairwise_target(args, feat, target, device):
    ...
    if target is None:
        ...
        rank_idx1, rank_idx2 = PairEnum2D(rank_idx)  # 4096, 4096 这两个都是这个尺寸
        rank_idx1, rank_idx2 = rank_idx1[:, :args.topk], rank_idx2[:, :args.topk]  # 4096,5  4096,5
...
def sigmoid_rampup(current, rampup_length):  # e−5(1− t/T )^2
...
class BCE_softlabels(nn.Module):  # 二值交叉熵损失损失
