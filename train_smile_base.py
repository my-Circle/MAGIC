...
class trainer(object):
    ...
    def build_model(self, opt):
        ...
        self.global_discriminator = d_cls_inst(fc_size=197376)  #fc_size得改 待调试 13312
        self.local_discriminator = d_cls_inst(fc_size=768)  #同上 256
    ...
    def get_batch_entropy_loss(self, p_softmax):
        entropy = - torch.mul(p_softmax, torch.log(p_softmax + 1e-5))
    ...
    def train(self, opt):
        ...
        for step in range(start_iter, opt.num_iter + 1):
            ...
            # Add domain loss
            loss = src_char_cls_loss.mean() + src_bpe_cls_loss.mean() + src_wp_cls_loss.mean() + 3 * omega * (d_img_loss.mean() + d_inst_loss.mean()) + tar_em_loss.mean() * tar_lambda #* 3
            loss_avg.add(loss)
            ...
            if step % opt.valInterval == 0:
                ...
                print(
                    f'[{step}/{opt.num_iter}] Loss: {loss_avg.val():0.5f} CLS_Loss: {cls_loss_avg.val():0.5f} SIMI_Loss: {sim_loss_avg.val():0.5f} EM_Loss: {em_loss_avg.val():0.5f} elapsed_time: {elapsed_time:0.5f}')
                # for log
